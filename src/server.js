...
app.get('/game', (req, res) => {
    const state = gameState.getState();
    console.log('当前游戏状态:', state);  // 添加日志
    
    if (!state.gameStarted || !state.playerName) {
        console.log('游戏未开始或玩家名字未设置');  // 添加日志
        return res.redirect('/');
    }
    
    const currentLocation = locations.getLocation(state.currentLocation);
    if (!currentLocation) {
        console.error('无效的位置:', state.currentLocation);  // 添加日志
        gameState.resetGame();
        return res.redirect('/');
    }
